<template>
  <div class="bg-white p-8 max-w-2xl mx-auto mt-8">
    ...
    <form @submit.prevent="submitProduct" v-if="currentPhase === 2">
      ...
      <div class="mt-8 text-center">
        <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded-md text-lg font-semibold hover:bg-blue-600 transition-colors duration-200">
          {{ isEditMode ? 'Actualizar Producto' : 'Añadir Producto' }}
        </button>
        <button type="button" @click="resetForm" class="ml-4 bg-gray-300 text-gray-700 px-6 py-2 rounded-md text-lg font-semibold hover:bg-gray-400 transition-colors duration-200">
          Cancelar
        </button>
      </div>
    </form>
